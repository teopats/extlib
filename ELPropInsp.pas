...
uses
  Classes, Controls, Grids, Graphics{$IFDEF D6_UP}, Types{$ENDIF}, Windows,
  Messages, Forms, StdCtrls, ELControls, SysUtils, Dialogs, TypInfo
  {$IFDEF D6_UP}, Variants{$ENDIF}, ComCtrls, Menus, ELSConsts
  {$IFDEF D5}, ELD5_Adds{$ENDIF}, ELStringsEdit;
...
procedure TELCustomPropsPage.CreateParams(var Params: TCreateParams);
begin
  inherited;
...
procedure TELCustomPropsPage.SetGridLines(const Value: boolean);
begin
  FGridLines := Value;
  Invalidate;
end;
...
begin
  ...
  if not FChangingBounds then
    begin
      ...
      try
        UpdateLoc(Rect(Left+1, Top+1, Left + Width, Top + Height - 2));
...
procedure TELVariantPropEditor.SetValue(const Value: string);
  ...
  begin
    ...
    if ANewType = varCurrency then
      Result := true; //AnsiPos(CurrencyString, AValue) > 0;
    if Result then
      try
...
begin
  ...
  if Value = '' then
    ...
    LV := NULL
  else if not _Cast(LV, LOldType) then
    LV := Value;
  SetVarValue(LV);
end;

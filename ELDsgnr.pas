...
uses
  ...
  {$IFDEF D6_UP}, RTLConsts{$ENDIF}, Clipbrd, ELControls;
...
type
  ...
  protected
    ...
    property PixelStep: Byte read FPixelStep write FPixelStep;
                      ...
                      begin
                        ...
                        if FSelCtrls.Count > 0 then
                          begin
                            ...
                            LdY := 0;
                            case TWMKeyDown(Message).CharCode of
                              VK_LEFT: LdX := -FDesigner.PixelStep;
                              VK_UP: LdY := -FDesigner.PixelStep;
                              VK_RIGHT: LdX := FDesigner.PixelStep;
                              VK_DOWN: LdY := FDesigner.PixelStep;
                            end;
            ...
            begin
              with LDrawRect do
                if (Right - Left > 1) or (Bottom - Top > 1) then
                  ...
                  SetBounds(Left, Top, LInsertingControl.Width, LInsertingControl.Height);

              if FDesigner.SnapToGrid and false then
                begin
                  Left := Left div FDesigner.Grid.FXStep * FDesigner.Grid.FXStep;
                  Top := Top div FDesigner.Grid.FYStep * FDesigner.Grid.FYStep;
                  Width := Width div FDesigner.Grid.FXStep * FDesigner.Grid.FXStep;
                  if not (LInsertingControl is TCustomEdit) then
                    begin
                      Height := Height div FDesigner.Grid.FYStep * FDesigner.Grid.FYStep;
                    end;
                end;
...
procedure TDEng.UpdateCaption(AVisible, AUpdateFrame: Boolean);
begin
...
function TDEng.GetExcludedStyleElements(
  AControl: TControl): TStyleElements;
begin
  Result := [];
end;
...
{$IFDEF WIN64}
    function FindScanline(Source: Pointer; MaxLen: NativeUInt;
      Value: Byte): NativeUInt; assembler;
    asm
      .NOFRAME
      MOV     RAX, RDX    // MaxLen
      MOV     RDX, R8     // Value
      MOV     R8, RCX     // Save Source
      MOV     RCX, RAX    // MaxLen to RCX for REP
      MOV     RAX, RDX    // Value to AL for SCASB
      MOV     RDI, R8     // Source to RDI
      REPNE   SCASB
...
begin
  ...
  if (Message.Msg = WM_LBUTTONDOWN) and (FSelCtrl.Mode = imSizeable) then
    begin
      ...
      LEng.IsDesignMsg(FSelCtrl.Control, PMessage(@LDMSng)^);
    end;
end;
...
begin
  AResult.Clear;
  for LI := 0 to Count - 1 do
    if Items[LI] <> FDesigner.DesignControl then

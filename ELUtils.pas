...
type
  TELDfrmFormat = (fmtBinary, fmtText);

procedure ELFormSaveToFile(Form: TForm; const FileName: string; Format: TELDfrmFormat);
procedure ELFormLoadFromFile(Form: TForm; const FileName: string; Format: TELDfrmFormat);
...
type
  TELControlAccess = class(TControl);
        ...
        fmtText:
          begin
            ...
            try
              ...
              FileStream.WriteBuffer(PChar(StringStream.DataString)^, Length(StringStream.DataString));
            finally
              StringStream.Free;
            end;
          end;
  ...
  try
    ...
    try
      case Format of
        ...
        fmtText:
          begin
            SetLength(TextBuffer, FileStream.Size);
            FileStream.ReadBuffer(PChar(TextBuffer)^, FileStream.Size);
            StringStream := TStringStream.Create(TextBuffer);
            try
              ObjectTextToBinary(StringStream, MemStream);
            finally
              StringStream.Free;
            end;
          end;
      ...
      // Read the form and its components
      MemStream.ReadComponent(Form);
      ...
      begin
        ...
        begin
          if TELControlAccess(Component).Parent = nil then
            TControl(Component).Parent := Form
          else if TELControlAccess(Component).Parent.Owner <> Form then
            TControl(Component).Parent := Form;
        end;
      end;
    finally
